...
function getRate(model: string): ModelRate {
  ...
  if (model.includes('sonnet')) return MODEL_RATES['claude-sonnet-4-5-20250929'];
  return DEFAULT_RATE;
}
...
export function logInvocation(db: Database.Database, data: InvocationData): void {
...
export function getMonthSummary(db: Database.Database): {
  ...
  const end = new Date(now.getFullYear(), now.getMonth() + 1, 1).toISOString().slice(0, 16);
...
export function getTodayDetail(db: Database.Database): Array<{
  ...
  ).all(`${today}T00:00:00`, `${today}T23:59:59`) as ReturnType<typeof getTodayDetail>;
}

export function getWeekSummary(db: Database.Database): Array<{ date: string; cost: number; count: number }> {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const row = db.prepare(
      `SELECT COALESCE(SUM(cost_usd), 0) as cost, COUNT(*) as count
       FROM cost_log WHERE timestamp >= ? AND timestamp < ?`,
    ).get(`${date}T00:00:00`, `${date}T23:59:59`) as { cost: number; count: number };
...
export function formatMonthlySummary(s: ReturnType<typeof getMonthSummary>): string {
  const lines = [
  ...
  if (s.bySource.length > 0) {
    ...
    for (const row of s.bySource) {
      lines.push(`  ${bar(row.cost, maxSrc)} ${row.source} $${row.cost.toFixed(4)} (${row.count}x)`);
    }
    lines.push('');
  }
  if (s.byModel.length > 0) {
    ...
    for (const row of s.byModel) {
      ...
      lines.push(`  ${bar(row.cost, maxMod)} ${short} $${row.cost.toFixed(4)} (${row.count}x)`);
    }
  }
...
export function formatTodayDetail(rows: ReturnType<typeof getTodayDetail>): string {
  if (rows.length === 0) return '📊 No invocations today.';
  const lines = ['📊 Today\'s invocations:'];
  for (const row of rows) {
    const time = row.timestamp.slice(11, 16) + 'Z';
    const dur = row.durationMs != null ? ` ${(row.durationMs / 1000).toFixed(1)}s` : '';
    const short = row.model.replace('claude-', '').slice(0, 20);
    lines.push(`  ${time} ${row.source} ${short} $${row.cost.toFixed(5)}${dur}`);
  }
...
export function formatWeekSummary(days: ReturnType<typeof getWeekSummary>): string {
  const lines = ['📊 Last 7 days:'];
  const maxCost = Math.max(...days.map((d) => d.cost), 0.0001);
  for (const day of days) {
    lines.push(`  ${day.date} ${bar(day.cost, maxCost)} $${day.cost.toFixed(4)} (${day.count}x)`);
  }
...
export function formatHeartbeatSummary(rows: ReturnType<typeof getHeartbeatSummary>): string {
  ...
  for (const row of rows) {
    ...
    lines.push(`  ${bar(row.cost, maxCost)} ${id} $${row.cost.toFixed(4)} (${row.count}x)`);
  }
  const total = rows.reduce((s, r) => s + r.cost, 0);
  lines.push(`Total: $${total.toFixed(4)}`);
  return lines.join('\n');
